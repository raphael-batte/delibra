...
(function () {
  ...
  (function watch() {
    ...
    var WATCH = ['gallery.html', 'gallery.js', 'gallery-diff.js', 'src-slice.js', '_frame.html', 'brand.js']
  ...
  var galleryDiff = window.ENGINE_GALLERY_DIFF.create({
    t: t,
    B: B,
  ...
  window.GALLERY_API = {
    setDiff: galleryDiff.setDiff,
    isDiffOn: galleryDiff.isDiffOn,
    spy: function () { pickActive(); return activeId || null; },
    setCompareCss: galleryDiff.setCompareCss,
    setDefaultTitle: setDefaultTitle,
    openSource: openSourceBadge,
    hasCompareCss: galleryDiff.hasCompareCss,
    diffSection: function (id) {
      if (!galleryDiff.isDiffOn()) galleryDiff.setDiff(true);
      var boxes = document.querySelectorAll('#' + id + ' .g-example');
      Array.prototype.forEach.call(boxes, galleryDiff.enqueue);
      return boxes.length;
    },
    frames: function () {
  ...
  nav.addEventListener('click', function () {
    if (galleryDiff.isDiffOn()) galleryDiff.setDiff(false);
  });
